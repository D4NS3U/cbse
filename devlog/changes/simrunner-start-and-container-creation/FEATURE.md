...
The reference Translator runs a rootless BuildKit sidecar (`moby/buildkit:v0.32.2-rootless`). Rootless `buildkitd` must run as the mapped root inside a user namespace: the Operator sets `hostUsers: false` on the Translator Pod and runs the `buildkit` container as `runAsUser: 0` with `--oci-worker-no-process-sandbox`, an unconfined seccomp profile, and an unconfined AppArmor profile (no privileged mode, host networking, host paths, host runtime sockets, or privilege escalation). This requires the cluster to have the `UserNamespacesSupport` feature gate enabled on the kube-apiserver, kube-controller-manager, and kubelet. The gate is beta in Kubernetes 1.30–1.32 (disabled by default) and GA in 1.33; on a cluster where it is off, the API server strips `hostUsers: false` and rootless `buildkitd` fails at startup (`can't enable NoProcessSandbox without Rootless`). Enabling the gate is a one-time cluster-admin operation the CBSE harness and Operator never perform; the experiment namespace must also permit the unconfined seccomp and AppArmor profiles (Pod Security enforce `privileged`, or an equivalent exception). `docs/CLUSTER_REQUIREMENTS.md` states the exact enablement steps, the kernel and containerd prerequisites, and the verification procedure, and is the source a future Helm chart's prerequisites must reference.
