...
3. **Reference Translator template and on-demand image creation.**
   ...
   - Translator publishes the ready message and waits for publish confirmation before acknowledging the request. A generator, BuildKit, push, or digest-resolution failure publishes the existing ready message with an empty image and is acknowledged only after that publish succeeds. NATS transport or ready-publication failures are retryable and must not manufacture an empty result.
...
- timeout recovery, cancellation, or self-healing beyond the fixed per-index Kubernetes retry budget;
...
7. SM RBAC and `test/e2e/manifests/` -- grant `get`, `list`, and `watch` for alpha4 experiments; `create`, `get`, `list`, and `watch` for Jobs; and `get`-only access, without `list` or `watch`, for Secrets and ServiceAccounts in the watched scope. SM must use direct named reads for the experiment's registry Secret and deterministic runner ServiceAccount. Provide digest-pinned Builder and Translator images plus the configured digest-pinned runner smoke base image. Translator generates the scenario-specific runner images; there is no prebuilt runner smoke image.
...
When the project table does not exist, SM creates it with the alpha4 schema. When it already exists, SM validates it without issuing `ALTER TABLE`. A legacy or incompatible project table makes startup fail before the informer, NATS consumers, or selector starts. Alpha4 is an unused breaking alpha API and defines no database migration procedure; an affected operator must independently provide a fresh compatible database or recreate the tables.
...
After a successful simulation, each runner appends one JSONB `result` containing exactly `scenario`, `seed`, `events_processed`, and `simulated_time`. PostgreSQL's sequence orders concurrent inserts. Completion indexes are not written to the Result DB, and results have no repetition identifier or uniqueness constraint. A simulation failure performs no result insert. A Result DB connection, table-creation, or insert failure makes the runner exit unsuccessfully and consumes one Pod attempt for that completion index. Kubernetes may then create a replacement Pod for the same index until its four-retry budget is exhausted.
...
After BuildKit confirms a successful push, Translator resolves the digest and verifies the repository and all three annotations. It then atomically writes a credential-free `ready-outcome.json` in the attempt workspace containing scenario ID, translation attempt, deterministic tag, digest, ready subject, and full experiment UID. It retains that workspace until the ready publish and server-confirmed request acknowledgement succeed. A publish retry validates and reuses this exact outcome without another registry lookup.

On redelivery or process restart, Translator first validates the marker's experiment UID, scenario ID, attempt, tag, digest, and ready subject. If the marker was lost because the Pod was replaced, Translator resolves the deterministic registry tag. It treats the tag as the successful build outcome only when the normalized repository and all three manifest annotations exactly match the current request. A missing or mismatched annotation is a permanent identity collision: Translator publishes the empty-image failure, does not invoke the generator or BuildKit, and never adopts or overwrites the tag. A registry not-found result permits generation and build; another resolution failure follows the empty-image failure workflow.
...
- the namespace-aware request and ready subjects, durable-consumer name and exact consumer settings, unchanged request and ready JSON payloads, serial delivery, 30-second in-progress acknowledgements, publish-confirmation-before-server-confirmed-acknowledgement rule, and permanent poison-message handling;
...
- pushed-outcome persistence and recovery, including marker validation, annotation-checked tag recovery, collision refusal, and the rule that a ready-publish retry always reuses the already-pushed digest and never rebuilds that attempt;
...
`spec.runner.jobTemplate` is optional. It is a typed `batch/v1.JobTemplateSpec`, not raw YAML and not a ConfigMap containing a Job. The built-in template is used when the field is absent. A supplied template must contain exactly one container named `runner`. All ordinary `batch/v1` Job and Pod-template fields are permitted unless they conflict with the explicit CBSE-owned identity, lifecycle, executable, or security fields below. This includes sidecars, init containers, runner environment and resources, volume mounts, ConfigMap or Secret volumes, additional image pull Secrets, user labels and annotations, `activeDeadlineSeconds`, suspension, node selectors, tolerations, affinity, topology constraints, and other scheduling settings.

SM constructs an effective Job by copying permitted template fields and then enforcing the fields owned by CBSE. Users cannot set or override the Job name, namespace, owner references, reserved identity labels or annotations, selectors or `manualSelector`, `managedBy`, TTL-after-finished, `completionMode: Indexed`, completions, parallelism, `backoffLimitPerIndex: 4`, global `backoffLimit: math.MaxInt32`, absent `maxFailedIndexes`, Pod `restartPolicy: Never`, runner image, runner command or arguments, required registry image pull Secret, runner ServiceAccount, disabled service-account token mounting, or required security settings. `podFailurePolicy`, `successPolicy`, and other settings that alter the protected completion or retry contract are also prohibited. A template may add distinct image pull Secrets for auxiliary images, but it cannot remove or replace the experiment registry Secret. The Operator rejects a template that conflicts with these fields and moves the experiment to provisioning `Error`; SM repeats the same validation before any Job create call and treats a conflict that escapes provisioning as a permanent scenario startup failure.
...
SM computes a deterministic contract hash and stores it as `experiment.cbse.terministic.de/job-contract-sha256`. The hash is the lowercase hexadecimal SHA-256 of JSON containing the finalized Job labels, user annotations, and effective Job specification after CBSE overrides. The hash input excludes the Job name, namespace, owner references, status, API-server metadata, and the hash annotation itself. The same effective-Job builder produces the hash input at creation and recovery time.

An `AlreadyExists` result is an expected recovery case when Kubernetes accepted Job creation but SM restarted or lost the subsequent Core DB transition. SM accepts the existing Job only when the contract-hash annotation, experiment owner UID, identity labels, and all protected live fields match. It never adopts or rewrites a mismatch.
...
A namespace-scoped SM uses a Role and RoleBinding in its watched namespace. A cluster-wide SM uses a ClusterRole and ClusterRoleBinding with the same resources and verbs. Secret and ServiceAccount reads are direct named `GET` operations for the references being validated; SM cannot enumerate or watch either resource. SM receives no Pod, ConfigMap, Job update, Job patch, Job delete, or RBAC-management permission. ConfigMap and Secret volumes other than the required registry Secret are resolved by Kubernetes and do not require SM to read those objects. Per-experiment runner ServiceAccounts have no RoleBinding and set `automountServiceAccountToken: false`.

Before starting informers, NATS consumers, runner selection, or observation, SM uses Kubernetes authorization checks to verify every required verb, resource, and namespace scope above. A denied check is a fatal installation error and prevents startup. If a later Kubernetes operation returns HTTP `403 Forbidden`, installed authorization has drifted from the required contract. SM logs the verb, resource, and namespace, leaves scenario and experiment state unchanged, returns the fatal error to its process boundary, and exits. Kubernetes may restart the process after installation RBAC is repaired; SM never translates `Forbidden` into scenario `Failed` or experiment `Error`.
...
3. **Scenario Manager startup.** SM fails startup for a malformed Core DB DSN, incompatible required table schema, invalid watch namespace, invalid NATS subject template, invalid configured stream or consumer name, or a denied required Kubernetes authorization check. It reports the exact setting, schema check, or denied verb/resource/scope before starting informers, consumers, scenario selection, or runner observation.
4. **Runtime dependencies.** A syntactically valid but unavailable NATS server or Kubernetes API is a retryable dependency failure. A registry, BuildKit, generator, push, or digest-resolution failure while handling a Translator request follows the confirmed empty-image workflow. A Result DB failure makes the current indexed runner Pod fail and consumes one attempt for that index. Kubernetes `Forbidden` is a fatal installation invariant violation that terminates SM without a domain-state transition. None of these failures is relabeled as a startup configuration error.
...
`number_of_reps` must be between `1` and `math.MaxInt32`, and `number_of_computed_reps` must initially be zero for this execution. The runner digest and template must be valid before Job creation. SM performs direct named `GET` operations to require the experiment's registry Secret with its correct type and key and the deterministic runner ServiceAccount. The Job includes the experiment registry Secret in `imagePullSecrets`; a custom template may add other distinct pull Secrets but cannot remove or replace the required one.
...
Cancellation, deadlines, Kubernetes transport errors other than `Forbidden`, and an experiment in an empty, `Pending`, or `Provisioning` phase are retryable. SM returns an error and leaves the scenario in `StartingRunners`. `Forbidden` leaves the scenario unchanged and terminates SM as a fatal installation error.
...
- For every observation, SM atomically sets `number_of_computed_reps` to `GREATEST(current_value, LEAST(job.status.succeeded, number_of_reps))` while guarding scenario ID and state `InProcessing`. For an indexed Job, `job.status.succeeded` represents successfully resolved completion indexes. The value never decreases or exceeds `number_of_reps`, so informer redelivery, Pod retry, and SM restart cannot double-count an index.
...
- `Forbidden` leaves the row unchanged and terminates SM through the fatal installation-error behavior above.
...
Smoke preflight requires the configured registry cleanup adapter, readable credentials, and runner smoke base-image digest before mutation. Cleanup attempts every recorded image even when one deletion fails. Cleanup failure is written to the run artifacts and fails an otherwise successful smoke run; on an already-failed run it is reported in addition to the original failure. Selection of the concrete registry, its deletion implementation, and the runner smoke base image is deliberately deferred until the test registry is supplied. Core API, Operator, SM, and Translator implementation may proceed without inventing those provider-specific details, but the full smoke suite and final feature acceptance remain blocked until this section names the registry, supported tag or manifest deletion API, authentication method, cleanup tool or client, and base-image digest.

### Verification and shutdown

Test alpha4-only CRD guidance; namespace/name/UID project identity; same-UID registration; different-UID replacement; stale UID update and deletion; DNS-label project-name validation; and the required Core DB schema failure for old tables. Verify that `scenario_status` receives no new identity columns. Test each named provisioning, Translator-startup, SM-startup, runtime-dependency, message, and scenario-work validation class above and assert its exact failure classification. Test Builder resource-limit defaults and valid user overrides. Test the absent default runner template and custom typed templates with active deadlines, suspension, sidecars, init containers, ConfigMap and other volumes, additional pull Secrets, runner mounts, resources, and standard scheduling constraints. Verify that the experiment registry pull Secret is retained. Reject every protected-field override, missing or duplicate `runner` containers, and non-root violations.
...
Unit-test the template's runtime configuration, exact durable-consumer configuration, request and Result DB validation, generator invocation, serial processing, deterministic workspace and tag construction, BuildKit calls, registry authentication, digest and OCI identity-annotation validation, empty-image failure publication, 30-second in-progress acknowledgements, confirmed ready publication, and server-confirmed request acknowledgement ordering. Reject an incompatible existing durable without modifying it. If an in-progress acknowledgement fails, verify cancellation and redelivery. After a successful push, force ready publication to fail and verify that retries and redelivery reuse the validated retained digest with zero additional registry lookup, generator, build, or push calls. Repeat after a Translator process restart. After loss of the Pod-local marker, verify that a tag with matching repository and annotations is reused and that a missing, mismatched, or unannotated tag is refused without rebuilding or overwriting it. Cover each exact Translator startup setting and mounted path listed above. Verify that Secret changes are not reloaded during the process lifetime. Test valid and invalid example `recipe_info`; inspect the generated SimPy build context for its baked scenario and Result DB configuration, non-root entrypoint, per-scenario PostgreSQL result sink, and absence of logged credentials. Verify concurrent append-only inserts produce the expected row count, a controlled simulation failure writes no row, and an ambiguous committed insert followed by retry may produce a duplicate. Verify that Translator and BuildKit can write their shared workspace as UID/GID `1000` with `fsGroup: 1000`.

Replace the synthetic Translator mock in the mandatory full smoke path with the reference template. The smoke path must demonstrate request consumption, example SimPy context generation from the configured digest-pinned runner smoke base image, a rootless sidecar build selected by alpha4, authenticated registry push through the alpha4 registry Secret, immutable digest publication, runner Job creation, non-root model execution, PostgreSQL result persistence, and the scenario transition through `InProcessing` to `PostProcessing`. The smoke Result DB is PostgreSQL. Configure at least two repetitions, verify they run as distinct completion indexes through one indexed Job, query exactly that many rows from `scenario_<scenario-id>_results` in the no-failure path, and verify `number_of_computed_reps = number_of_reps`.

Test the exact Translator sidecar, default and overridden resource limits, volumes, UID/GID `1000` workspace access, socket, serial request handling, digest publication, empty-image failure handling, repository-mismatch rejection, OCI identity collisions, and request acknowledgement ordering. Verify that Baseline and Restricted admission reject the required BuildKit profiles, that the smoke namespace uses the explicit admission exception, and that all runner-workload containers remain non-root and non-privileged. Test Job naming, labels, owner reference, indexed manifest shape, contract-hash annotation and input exclusions, `N` parallel indexed completions, `backoffLimitPerIndex: 4`, global `backoffLimit: math.MaxInt32`, absent `maxFailedIndexes`, matching `AlreadyExists`, collision refusal, and Job-before-state ordering. Verify success after transient indexed failures, five total failed Pod attempts for one index, continued execution of other indexes, terminal failure when any index exhausts retries, and preservation of successful-index counts. Test active, partial-success, successful, failed, missing, stale, and contradictory Job observations; monotonic repetition updates; redelivery and restart safety; and preservation of partial counts on failure. Confirm that runner observation cannot block ordinary scenario selection. Include a maximum-valid `math.MaxInt32` repetition manifest unit test without creating Pods.

Test the resource-neutral internal scheduler interface with a fake adapter and the built-in Kubernetes Job adapter. Assert the exact namespace-scoped and cluster-wide RBAC resources and verbs, including `get` without `list` or `watch` for Secrets and ServiceAccounts. Denied startup authorization must prevent informers, consumers, selection, and observation from starting. Simulate Kubernetes `Forbidden` for Job creation and observation, verify that no scenario or experiment transition occurs, and verify that SM reports the verb, resource, and namespace before terminating through its process boundary.

Test lightweight scenario observability without requiring a logging backend: SM emits one creation or adoption record and one terminal record with the required scenario fields, each failed runner Pod attempt emits one sanitized failure record, a controlled simulation failure writes no result row, success does not emit one framework log per repetition, and no credential or full recipe payload appears. Keep Pod-level startup failures available through standard Kubernetes diagnostics. Log retention and automatic deletion receive no implementation test in this branch.

The repository test pipeline must import only alpha4 experiment types in active Operator, SM, envtest, compatibility, and smoke paths. Generated CRDs, samples, fixtures, schemes, RBAC, preflight storage-version checks, and smoke assertions use alpha4. Tests assert alpha2 and alpha3 are not served or reconciled and never treat alpha4 as deprecated or transitional. Image publication builds the reference Translator instead of `trans-mock` and supplies the configured Builder digest, runner smoke base-image digest, and registry Secret. Once the deferred registry contract is supplied, an unconditional cleanup test proves that generated runner images are removed after both a successful smoke run and a forced failure without deleting shared component or base images.
...
After implementation, run `make test-fast`. Once the deferred registry provider, cleanup API and client, credentials, and runner smoke base-image digest have been supplied, run the mandatory `make test-smoke` with an explicit `KUBECONFIG`, immutable test image digests, and the configured registry-auth file. Smoke preflight must require the already-installed alpha4-only CRD and must not delete, migrate, or downgrade the shared CRD. Full feature acceptance remains blocked until this smoke suite can run and passes. Do not add test artifacts to commits.
