...
3. Read [`../IMPLEMENTATION_HANDOFF.md`](../IMPLEMENTATION_HANDOFF.md) completely. Treat it as routing evidence, not proof of completion.
4. Read the [slice dependency map](../FEATURE.md#slice-dependency-map) and [sequential completion contract](../FEATURE.md#sequential-completion-contract). Use the handoff and repository evidence to identify the earliest slice that is not complete. Read that target slice and every deferred group assigned to it completely before changing code. Read an earlier slice completely only when its evidence is missing, the target changes its contract, or a failure points back to it. If taking a second slice, read it completely only after the first has passed its completion gate.
...
- the first slice satisfies its local completion gate and every deferred group assigned to it by `FEATURE.md`;
...
- Validate the handoff's base commit, target, state, changed paths, and test evidence against the current worktree. A stale or inconsistent handoff is corrected before implementation begins.
- Compare implementation, generated artifacts, tests, and documentation against the target's stable milestones, local acceptance groups, incoming deferred groups, and `Completion and handoff` section.
- Use code and test evidence to classify each inspected slice as:
  - `complete`: every local milestone, local acceptance group, and incoming deferred group is implemented, required generated output is current, and every required test tier has passing evidence tied to the current revision or unchanged worktree;
...
- Record a complete slice as `smoke-verified` in the handoff. Record unavailable mandatory verification as `blocked`, while reporting the slice as `verification-blocked` in the final response.
...
- Update `IMPLEMENTATION_HANDOFF.md` with the current base commit, target milestone, and `in-progress` state before implementation. Multiple runs within one large slice are expected; resume at the first incomplete stable ID.

## Implementation rules

- Implement the target slice's local milestones, local acceptance groups, incoming deferred groups, owned tests, generated artifacts, and documentation. Requirements are not optional merely because they are large.
...
4. Update `IMPLEMENTATION_HANDOFF.md` with the exact state reached, completed stable IDs, sanitized verification evidence, first incomplete ID, and next action.
...
1. `Slice status` — list every slice inspected and classify it as `complete`, `incomplete`, or `verification-blocked`; name the slice and stable IDs worked on.
...
7. `Resume at` — name exactly one stable milestone or acceptance-group ID and the first remaining task, or state that all seven slices are complete.
