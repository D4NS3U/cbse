...
> **Status:** **Consolidated back into the single repository (user decision 2026-09-25; D2 superseded).** All governance (charter, specs, handoffs, prompts, MANAGER.md, agent docs) has returned to this repository from the short-lived private cbse-labs; the private cbse-labs repository was frozen and deleted (its local clone remains as archive). The public tree carries the sharpened docs surface of Package A (product README, OSS staples, alpha4-only docs, headers) beside the returned process records — an accepted position for a development-stage research repo, matched by CONTRIBUTING's stage notice. Program state: Package A complete + M0 passed (deviations documented); Package B parked by the user (images stay private); Package C parked with it. The self-hosted cluster-smoke workflow is restored to `.github/workflows/` (runner registration on the K3s box pending, user-external). Governance updates live in `devlog/changes/public-repo-hygiene/IMPLEMENTATION_HANDOFF.md`.
