...
| --- | --- | --- |
| [`repo-professionalization/`](repo-professionalization/) | Program charter (`PROBLEM.md`): turn the dev-heavy repository into a professional open-source-ready product repo. Manager-authored `FEATURE.md` program in three packages (public hygiene, public release pipeline, dev-box sunset) — not a worker-facing spec yet | **In progress — Package A decomposed and dispatching** (D1: alpha2/alpha3 deletions confirmed; `test/compat/eds-sm/` + `hack/` pending confirmation; D3: relocate, preservation staged via A4; B/C milestone-locked) |
| [`public-repo-hygiene/`](public-repo-hygiene/) | Package A of the repo-professionalization program ([charter](repo-professionalization/PROBLEM.md)): OSS staples (A1), retired-tree removal (A2), private-infra scrub of tracked defaults and agent-facing prose (A3), cbse-labs preparation manifest (A4) — umbrella [FEATURE.md](public-repo-hygiene/FEATURE.md) | **complete and reviewer-verified (2026-09-24): A1–A4 + residual deletions + public cluster-smoke.yml removal + header pass (L1) + SECURITY/org (S2) + LICENSE appendix (L2); open A user decisions: none; remaining: user commit plan, D2 (cbse-labs), M0 fresh-clone pass** |
