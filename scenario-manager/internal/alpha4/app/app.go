...
import (
	...
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/config"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/informer"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/jobadapter"
	...
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/natsadapter"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/observation"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/persistence"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/rbac"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/ready"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/runnerstart"
	"github.com/D4NS3U/cbse/scenario-manager/internal/alpha4/selection"
...
func RunScenarioManager(ctx context.Context) {
	...
	msgCleaner := lifecycle.NewNATSDeletionClient(js)
	...
	adapters := natsadapter.NewAdapters(nc, js, k8sClient, db, readyHandler.Handle)
	publisher := natsadapter.NewTranslationRequestPublisher(js)
	availability := natsadapter.NewEDSAvailabilityResponder(adapters)
	edsBatch := natsadapter.NewEDSBatchConsumer(adapters)
	translatorReady := natsadapter.NewTranslatorReadyConsumer(adapters)
...
func validatePureConfig(getenv func(string) string) (startupConfig, error) {
	...
	if err := messaging.ValidateTemplates(getenv); err != nil {
		return startupConfig{}, err
	}
	if err := messaging.ValidateStreamNames(getenv); err != nil {
		return startupConfig{}, err
	}
