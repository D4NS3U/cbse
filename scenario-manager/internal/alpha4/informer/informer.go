...
// bound to the alpha4 library: a DBProjectStore over the persistence store and
// a registerProject closure over persistence.RegisterProject. It is a
// convenience for the app wiring so the informer and the dispatcher share the
// same store and DB.
func NewProductionDispatcher(k8s client.Client, store persistence.Store, msg lifecycle.MessagingCleaner) *Dispatcher {
	...
	return NewDispatcher(k8s, projectStore, msg, func(ctx context.Context, namespace, project string) error {
