...
// The Kubernetes client, project store, messaging cleaner, and project
// registration are injected so the dispatch logic is unit-testable with fakes.
...
func (d *Dispatcher) runOnce(ctx context.Context, exp *experimentalpha4.SimulationExperiment, kind lifecycle.ActionKind) error {
	...
	case lifecycle.ActionDeletionCleanup:
		return lifecycle.RunDeletionCleanup(ctx, d.k8s, d.store, d.msg, exp)
	default:
