...
// delegated to messaging.ReconcileStreamsAndConsumers at startup; the adapters
...
// messaging.ReconcileStreamsAndConsumers at startup). k8s must have a scheme
...
// ACK, AckWait, and MaxAckPending from messaging.EDSConsumerConfig.
func edsConsumerOptions() []natsgo.SubOpt {
	cfg := messaging.EDSConsumerConfig()
...
func translatorReadyConsumerOptions() []natsgo.SubOpt {
	cfg := messaging.TranslatorReadyConsumerConfig()
	return []natsgo.SubOpt{
		natsgo.Durable(cfg.Durable),
		natsgo.ManualAck(),
		natsgo.AckWait(cfg.AckWait),
