...
// uidPrefix returns the canonical experiment-UID prefix: lowercase, hyphens
// stripped, first 12 characters. It mirrors nats.UIDPrefix (used for the
// Translator durable consumer name) so the deterministic runner Job name and
// ServiceAccount name agree with the consumer name without this transport-neutral
// package importing the NATS package.
...
func RunnerServiceAccountName(uid types.UID) string {
	return "simrunner-" + uidPrefix(string(uid))
}
